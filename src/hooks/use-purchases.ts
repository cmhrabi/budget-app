...
export const purchaseKeys = {
  ...
  analytics: (filters?: PurchaseFilters) =>
    [...purchaseKeys.all, 'analytics', { filters }] as const,
}
...
export function useCreatePurchase() {
  ...
  return useMutation({
    mutationFn: (purchase: CreatePurchaseRequest) => purchaseService.createPurchase(purchase),
    onSuccess: (newPurchase) => {
      // Invalidate and refetch purchases list
      queryClient.invalidateQueries({ queryKey: purchaseKeys.lists() })

      // Optionally update the cache with the new purchase
      queryClient.setQueryData(purchaseKeys.detail(newPurchase.id), newPurchase)
...
export function useOptimisticPurchaseUpdate() {
  ...
  const optimisticallyUpdatePurchase = (id: string, updates: Partial<Purchase>) => {
    queryClient.setQueryData(purchaseKeys.detail(id), (old: Purchase | undefined) =>
